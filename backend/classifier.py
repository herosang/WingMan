...
def infer(args, multiple=False):
    ...
    for img in rand_smpl:
        ...
        for r in reps:
            ...
            if multiple:
                print("Predict {} @ x={} with {:.2f} confidence.".format(prediction, bbx,
                                                                         confidence))
            else:
                print("Predict {} with {:.2f} confidence.".format(prediction, confidence))
...
def update_model(args, multiple=False):
    with open(args.classifierModel, 'r') as f:
        (le, clf) = pickle.load(f)

    encoded_labels = le.transform(args.labels)

    for index, img in enumerate(args.imgs):
        print("\n=== {} ===".format(img))
        reps = getRep(img, multiple)
        if len(reps) > 1:
            print("List of faces in image from left to right")
        for r in reps:
            ...
            clf.partial_fit(rep, [encoded_labels])
            print("Updated model.")
            fName = args.classifierModel
            print("Saving classifier to '{}'".format(fName))
            with open(fName, 'w') as f:
                pickle.dump((le, clf), f)
...
if __name__ == '__main__':
    ...
    updateModelParser = subparsers.add_parser('update_model',
                                        help="Train a new classifier iteratively.")
    updateModelParser.add_argument(
        'classifierModel',
        type=str,
        help='The Python pickle representing the classifier. This is NOT the Torch network model, which can be set with --networkModel.')
    updateModelParser.add_argument('imgs', type=str, nargs='+',
                             help="Input image.")
    updateModelParser.add_argument('labels', type=str, nargs='+',
                             help="Input image.")
